...
design/fonts.css      the inlined @font-face (UI sans) — generated by build/fonts.py
...
`CSS_PARTS`** (`design/fonts.css`, `design/tokens.css`, then `build/src/app.css`) into the
...
and tracked; data uses tabular figures. Lineage is signalled by a small glowing
dot (echoing the tree's nodes), not by coloured borders on the reading surfaces.
...
CI builds the workshop on every push to `main` and publishes it beside the app
at `/storybook/`. Nothing about it is committed, and its build step is
`continue-on-error`: dev-only tooling must never be able to block an app fix.
It is excluded in `robots.txt` — tooling, not site content.
